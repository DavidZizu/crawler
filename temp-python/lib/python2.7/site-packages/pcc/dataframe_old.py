...
class dataframe(object):
    ...
    def __init__(self, name = str(uuid4())):
        ...
        self.known_objects = self.object_map
    ...
    ################################################
    ## Needs type reload atomicity #################
    ################################################
    ## Taken care of by pausing/unpausing server ###
    ################################################
    ...
    def __add_type(self, tp, except_type = None, tracking=False, not_member=False):
        ...
        if (tp != keytp
            and (not dataframe.__is_impure(tp, categories))
    ...
    def __adjust_pcc(self, objs, groupname, original_changes = None):
        ...
        obj_map = dataframe.build_pccs(can_be_created, {groupname: objs}, None, self.impure)  
    ...
    @staticmethod
    def __construct_pccs(pcctype, pccs, universe, param, impures):
        if pcctype in pccs:
            return pccs[pcctype]
        ...
        dependent_pccs = [tp for tp in (dependent_types + paramtypes) if tp.__realname__ in impures or tp.__realname__ not in universe]
        ...
        for tp in to_be_resolved:
            dataframe.__construct_pccs(tp, pccs, universe, None, impures)
        ...
        pccs[pcctype] = dataframe.__make_pcc(pcctype, relevant_objs, param_collection_objs, params, hasSingleton, hasCollection)

    @staticmethod
    def build_pccs(pcctypes, universe, params, impures):
        pccs = dict()
        for pcctype in pcctypes:
            dataframe.__construct_pccs(pcctype, pccs, universe, params, impures)
        return pccs
    ...
    def get(self, tp, parameters=None):
        ...
        if not dataframe.__is_impure(tp, self.categories[tp.__realname__]) and tpname in self.object_map:
        ...
        objs = dict()
        objs = dataframe.build_pccs([tp], self.object_map, parameters, self.impure)
    ...
    def get_one(self, tp, oid):
        ...
        if not dataframe.__is_impure(tp, self.categories[tp.__realname__]) and tpname in self.object_map and oid in self.object_map[tpname]:
    ...
    # To only be called by master/client dataframes.
            ...
            for oid, obj_changes in groupchanges.items():
                ...
                for member, status in obj_changes["types"].items():
                    ...
                    if (status == Event.New or status == Event.Modification):
                        if member not in self.known_objects or oid not in self.known_objects[member]:
    ...
    def __generate_dim(self, dim_change, foreign_keys, built_objs):
        try:
            ...
            pass
        dim_type = dataframe.__get_obj_type(dim_change, self.member_to_group)
    ...
    def add_to_record_cache(self, event_type, tpname, oid, dim_change = None, already_converted = False):
        self.temp_record.put((event_type, tpname, oid, dim_change, already_converted))
...
class dataframe_client(dataframe):
    ...
    @staticmethod
    def _dataframe__is_impure(tp, categories):
        return False
...
class dataframe_cache(object):
    def __init__(self, df, name = str(uuid4())):
        self.master_df = df
        self.name = name
        self.object_map = RecursiveDictionary()
        self.current_state = RecursiveDictionary()
